...
| 字段 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `provider` | `"pagefind" \| "algolia"` | `"pagefind"` | 搜索引擎 |
| `algolia` | `AlgoliaConfig` | — | Algolia 配置，provider 为 `algolia` 时必填 |
...
| `apiKey` | `string` | 搜索 API Key（公开可用） |
| `indexName` | `string` | 索引名称 |

```ts
// 默认：使用 Pagefind（构建后自动生成索引）
search: {
  provider: "pagefind",
}
...
<Callout type="info" title="Pagefind 默认可用">
  Pagefind 在构建时自动生成搜索索引，无需额外配置。Algolia 集成即将推出。
</Callout>
